...
def selectStrike(df):
    ...
    dfd_ce = db.query("SELECT max(symbol) AS ce_sm FROM df_ltp WHERE ltp >= 150 AND ltp <= 200 AND option_type = 'CE'").df()
    dfd_pe = db.query("SELECT max(symbol) AS pe_sm FROM df_ltp WHERE ltp >= 150 AND ltp <= 200 AND option_type = 'PE'").df()
