...
"               on a.symbol=b.symbol")
    ...
    if (previous['20 CXvr'] or previous['MA20 SuP'] or latest['20 CXvr'] or latest['MA20 SuP']) and latest['Above ST11'] and latest['Above ST10'] and (latest['ATR (8)'] >= 8 or latest[f'ATR {arrow}']):
        ltp = df['LTP'].iloc[0]
        stop_loss = 8
        atr = latest['ATR (8)']
        target = 10 if 8 <= atr <= 12 else 15 if atr > 12 else 10
        # target = 15
        qty = 1
        symbol = symb
